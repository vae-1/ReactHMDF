...
class Trainer(object):
    ...
    def random_select(self, dtype_site_group_pid_clip_idx, test=False):
        ...
        # #添加调试信息
        # print(f"speaker_index: {speaker_index}, sim_speakers: {sim_speakers}")
    ...
    def load_npy(self, site_group_pid_clip_idx):
        ...
        idx = int(idx)
        listener_emotion_path = os.path.join('/home/lqc/test/data',
    ...
    def _parse_data(self, data, test=False):
        v_inputs, a_inputs, targets, s_inputs, targets_s = data
        v_inputs = v_inputs.cuda()
        a_inputs = a_inputs.cuda()
        s_inputs = s_inputs.cuda()
        if not test and self.neighbor_pattern == 'pair':
            return v_inputs, a_inputs, targets.cuda(), s_inputs, targets_s.cuda()
        ...
        # print("all_neighbors:",all_neighbors.shape)
        return v_inputs, a_inputs, s_inputs, all_neighbors, lengthes
    ...
    def train(self, epoch, dataloader, optimizer, print_freq=1, train_iters=100):
        ...
        for i, data in enumerate(dataloader):
            ...
            v_inputs, a_inputs, s_inputs, targets, lengthes = self._parse_data(data)
            ...
            if not self.no_inverse:
                speaker_features, listener_features, params, edge, nearest_targets, loss_det = \
                    self.model(v_inputs, a_inputs, s_inputs, targets, lengthes)
            else:
    ...
    def test(self, testloader, print_freq=1, modify=False, save_base=None):  # 定义测试方法，包含测试数据加载器、打印频率、是否修改和保存路径
        self.model.eval()  # 将模型设置为评估模式
        test_iters = len(testloader)  # 获取测试数据加载器中的批次数量
        self.modify = modify  # 设置是否修改输出
        for data in tqdm(testloader):  # 遍历测试数据加载器中的数据，使用tqdm显示进度条
            v_inputs, a_inputs, s_inputs, targets, _ = self._parse_data(data, test=True)  # 解析数据，获取视频输入、音频输入和目标标签
            # print(data)
            base_site_group_pid_idx = data[2][0].split('+')  # 提取基本站点组PID索引
            save_path = os.path.join(save_base, os.path.join(*base_site_group_pid_idx[:-1]))  # 构建保存路径
            mkdir_if_not_exist(save_path)  # 如果保存路径不存在，则创建

            with torch.no_grad():  # 在不计算梯度的上下文中进行预测
                for j in range(10):  # 遍历10次，每次生成一个预测结果
                    # cal_norm = True
                    # batch_size, num_frames, 25
                    predictions = self.model.inverse(v_inputs, a_inputs, s_inputs, cal_norm=True, threshold=self.threshold)  # 使用模型的逆过程进行预测
                    if self.modify:  # 如果需要修改输出
                        predictions = self.modify_outputs(predictions)  # 修改预测结果
                    predictions = self.combine_preds(predictions)  # 合并预测结果
                    torch.save(predictions, os.path.join(save_path, 'result-' + str(j) + '.pth'))  # 保存预测结果到指定路径
